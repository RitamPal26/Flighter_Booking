...
export default function RescheduleDialog({
    ...
    if (!result.success) {
      ...
      if (msg.includes("within 2 hours") || msg.includes("2 hours")) {
        toast.error("Cannot reschedule", {
          description:
            "Your current flight is within 2 hours of departure. Rescheduling is not permitted at this time.",
        });
      } else {
  ...
  const getRowNum = (seatNumber: string) =>
    parseInt(seatNumber.replace(/[A-F]/g, ""), 10);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "select_flight" &&
              "Pick a new date and flight for your rescheduled journey."}
                ...
                {flights.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-8">
                    {date
                      ...
                      : "Select a date to search flights."}
                  </p>
                ) : (
                  flights.map((flight) => (
                    <Card
        ...
        {step === "select_seat" && selectedFlight && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              ...
              {selectedFlight.destination} &middot;{" "}
              ...
              <>
                <div className="flex gap-4 text-sm justify-center flex-wrap">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 bg-purple-300 border border-purple-400 rounded"></div>{" "}
                    First
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 bg-sky-300 border border-sky-400 rounded"></div>{" "}
                    Business
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 bg-stone-200 border border-stone-300 rounded"></div>{" "}
                    Economy
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="overflow-x-auto pb-4">
                  <div className="min-w-[500px] space-y-6">
                    {["first", "business", "economy"].map((cls) => {
                      ...
                      const endRow = getRowNum(
                        clsSeats[clsSeats.length - 1].seat_number,
                      );

                      return (
                        <div key={cls}>
                          <div
                            className={`border-2 rounded-lg px-3 py-1.5 text-xs font-semibold uppercase tracking-wider mb-3 ${classTheme[cls]}`}
                          >
                            {classLabel[cls]} &middot; Rows {startRow}&ndash;
                            {endRow}
                          </div>
                          <div className="grid grid-cols-7 gap-2 text-center items-center justify-items-center">
                            {colHeaders.map((col, i) => (
                              <div
                                key={i}
                                className="font-bold text-gray-400 text-xs w-10"
                              >
                                {col}
                              </div>
                            ))}
          ...
          <div className="space-y-4">
            <div className="rounded-lg border bg-muted/30 p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {selectedSeat.seat_number} &middot;{" "}
